...
class ClientController extends Controller
{
    ...
    Public function  update(Request $request)
    {
        ...
        {
          $clients=  DB::select('select reservations.id as id ,agences.name as name,voitures.modele as modele,voitures.immatriculation as immatriculation,reservations.date_depart as date_depart ,reservations.date_retour as date_retour, action from users join reservations on reservations.id_client=users.id join voitures on voitures.id= reservations.id_voiture join agences on agences.id=voitures.location_id where users.id=?',[Auth::user()->id]);
